...
Процедура СоздатьСписокСправочниковДляЗагрузки(СписокСправочниковДляЗагрузки) Экспорт
	...
	//Корыткин А.А. доп. инфа
	МассивТипов = Новый массив;
	МассивТипов.Добавить(ТИп("ПланВидовХарактеристикСсылка.ТипыЭталонов"));
	ТипЭталон = Новый ОписаниеТипов(МассивТипов);
	ИнформацияОСправочниках.Колонки.Добавить("ВидЭталона", ТипЭталон);
	//
	...
	Для каждого ИнформацияОСправочнике Из ИнформацияОСправочниках Цикл
		...
		КонецЕсли;      
		//Корыткин А.А.
		ИнформацияОТипеЗагрузки.Вставить("ВидЭталона", ИнформацияОСправочнике.ВидЭталона);	
		//
...
Функция ПолноеИмяОбъектаТабличнаяЧасть(ИмяОбъекта) Экспорт
	...
	ИначеЕсли Результат.Количество() = 2 Тогда
		Если Результат[0] <> "Документ" ИЛИ Результат[0] <> "Справочник" ИЛИ Результат[0] <> "Обработка" Тогда  //Корыткин
		...
		Иначе
			...
			Иначе   
				...
				ТипОбъектаМетаданных = Метаданные.Обработки.Найти(НазваниеОбъектаМетаданных); //Корыткин
				Если ТипОбъектаМетаданных <> Неопределено Тогда 
					ТипОбъектаМетаданных = "Обработка";
				Иначе   
