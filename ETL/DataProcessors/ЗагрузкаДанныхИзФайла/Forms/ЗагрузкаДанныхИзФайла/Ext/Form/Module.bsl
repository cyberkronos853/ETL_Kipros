...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Реквизит_ТипЭталона = Новый РеквизитФормы("ТипЭталона",	Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.ТипыЭталонов"),	, "Ячейка");
    ДобавляемыеРеквизиты.Добавить(Реквизит_ТипЭталона);
...
Процедура ВыполнитьШагЗаполнениеТаблицыДаннымиНаСервере(ОписаниеСтрокиВыбора)
	...
	ЭтаФорма.ТипЭталона = ОписаниеСтрокиВыбора.ВидЭталона;
	...
	СформироватьМакетПоВидуЭталона(ЭтаФорма.ТипЭталона);    
	//...
...
&НаСервере
Процедура СформироватьМакетПоВидуЭталона(Эталон)
	
	ПараметрыЗагрузки = Новый Структура; 
	ПараметрыЗагрузки.Вставить("ВидЭталона", Эталон);
	Если ТипЗагрузки = "УниверсальнаяЗагрузка" Тогда
		ЭтотОбъект.АвтоЗаголовок = Ложь;
	ИначеЕсли ТипЗагрузки = "ПрикладнаяЗагрузка" Тогда
		ОпределитьПараметрыЗагрузки(ПараметрыЗагрузки);
	...
	Справочники.ЗначенияЭталонов.ДобавитьИнформациюОРеквизитах(ИнформацияПоКолонкамТаблица, Эталон);
...
Процедура СопоставитьДанныеПрикладнаяЗагрузка(ТаблицаСопоставленияДанныхСервер)
	...
	МенеджерОбъект.СопоставитьЗагружаемыеДанныеИзФайла(ТаблицаСопоставленияДанныхСервер, ЭтаФорма.ТипЭталона);
...
Функция МенеджерОбъекта(ИмяОбъектаСопоставления)
		...
		ИначеЕсли МассивОбъекта.ТипОбъекта = "Обработка" Тогда    //Корыткин
...
Процедура ЗаписатьСопоставленныеДанныеПрикладнаяЗагрузка(СопоставленныеДанные)
	...
	ПараметрыЗагрузки.Вставить("СоздаватьНовые", СоздаватьЕслиНеСопоставлено);
	ПараметрыЗагрузки.Вставить("ОбновлятьСуществующие", ОбновлятьСуществующие);
	МенеджерОбъекта.ЗагрузитьИзФайла(СопоставленныеДанные, ПараметрыЗагрузки, Отказ, ЭтаФорма.ТипЭталона);
