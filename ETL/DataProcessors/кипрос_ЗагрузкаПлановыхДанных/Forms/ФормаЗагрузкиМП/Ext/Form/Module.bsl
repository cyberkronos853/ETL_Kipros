...
Процедура ЗагрузитьФайлЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если (ВыбранныеФайлы <> Неопределено) Тогда				
		...
		Если Не СтрНайти(ИмяФайла,"CAPEX") = 0 Тогда
			ВидЗагружаемыхДанных = "CAPEX";
		ИначеЕсли Не СтрНайти(ИмяФайла,"BS") = 0 Тогда
			ВидЗагружаемыхДанных = "BS/PL/CFD";
		ИначеЕсли Не СтрНайти(ИмяФайла,"PL") = 0 Тогда
			ВидЗагружаемыхДанных = "BS/PL/CFD";
		ИначеЕсли Не СтрНайти(ИмяФайла,"CFD") = 0 Тогда
			ВидЗагружаемыхДанных = "BS/PL/CFD";
		КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	ВидЗагружаемыхДанных = "BS/PL/CFD";
	Если параметры.Свойство("КолонкиМаппингBS") 
		//ИЛИ параметры.Свойство("КолонкиМаппингCAPEX")
		Тогда
		//Если ВидЗагружаемыхДанных = "BS/PL/CFD" Тогда
		//	КолонкиМаппинг = Параметры.КолонкиМаппингBS;
		//ИначеЕсли ВидЗагружаемыхДанных = "CAPEX" Тогда
		//	КолонкиМаппинг = Параметры.КолонкиМаппингCAPEX;
		//КонецЕсли;
		Для Каждого СтрокаТЧ Из Параметры.КолонкиМаппингBS Цикл
			НоваяСтрока = КолонкиМаппинг.Добавить();
			НоваяСтрока.Вид = "BS/PL/CFD";
			НоваяСтрока.Имя = СтрокаТЧ.Имя;
			НоваяСтрока.Тип = СтрокаТЧ.Тип;
		КонецЦикла;
	КонецЕсли;
	Если параметры.Свойство("КолонкиМаппингCAPEX") Тогда	
		Для Каждого СтрокаТЧ Из Параметры.КолонкиМаппингCAPEX Цикл
			НоваяСтрока = КолонкиМаппинг.Добавить();
			НоваяСтрока.Вид = "CAPEX";
			НоваяСтрока.Имя = СтрокаТЧ.Имя;
			НоваяСтрока.Тип = СтрокаТЧ.Тип;
		КонецЦикла;
		ЗаполнитьМаппинг();
...
&НаСервере
Процедура ЗаполнитьМаппинг()
	МаппингЗагрузки.Очистить();
	Колонки = КолонкиМаппинг.НайтиСтроки(Новый Структура("Вид",ВидЗагружаемыхДанных));
	//Если ВидЗагружаемыхДанных = "BS/PL/CFD" Тогда
	//	КолонкиМаппинг = Параметры.КолонкиМаппингBS;
	//ИначеЕсли ВидЗагружаемыхДанных = "CAPEX" Тогда
	//	КолонкиМаппинг = Параметры.КолонкиМаппингCAPEX;
	//КонецЕсли;
	Для Каждого КолонкаМаппинг Из Колонки Цикл
...
Процедура Загрузить(Команда)
	ЗагрузитьНаСервере();
	Оповестить("ТаблицаЗагружена",Новый Структура("Адрес,ВидЗагружаемыхДанных",Адрес,ВидЗагружаемыхДанных));
	ЭтаФорма.Закрыть();
КонецПроцедуры
...
Процедура ЗагрузитьНаСервере()
	...
	ТаблицаВозврата.Колонки.Добавить("Год",ОписаниеТиповД,"Год");
	...
	Массив = Новый Массив;
	Массив.Добавить(Тип("СправочникСсылка.КатегорииАктивов"));
	ОписаниеТиповКатегорииАктивов = Новый ОписаниеТипов(Массив, , );
	Массив = Новый Массив;
	Массив.Добавить(Тип("СправочникСсылка.Доходы"));
	ОписаниеТиповДоходы = Новый ОписаниеТипов(Массив, , );
	Массив = Новый Массив;
	Массив.Добавить(Тип("СправочникСсылка.ТоварыИУслуги"));
	ОписаниеТиповТоварыИУслуги = Новый ОписаниеТипов(Массив, , );
	Массив = Новый Массив;
	Массив.Добавить(Тип("СправочникСсылка.ВидыДвижения"));
	ОписаниеТиповВидыДвижения = Новый ОписаниеТипов(Массив, , );
	Массив = Новый Массив;
	Массив.Добавить(Тип("СправочникСсылка.Валюты"));
	ОписаниеТиповВалюты = Новый ОписаниеТипов(Массив, , );
	...
	Для Каждого СтрокаТЧ Из ЭтаФорма["ТаблицаФормы"] Цикл
		...
		Для Каждого СтрокаМаппинга из МаппингЗагрузки Цикл
			Если ЗначениеЗаполнено(СтрокаМаппинга.КолонкаФайла) Тогда
				...
				Если Не СтрокаМаппинга.Тип.типы().Найти(Тип("Число")) = Неопределено Тогда
					текЗначение = СтрокаТЧ[СтрокаМаппинга.КолонкаФайла];
					Если СтрНайти(текЗначение,"E") = 0
						И СтрНайти(текЗначение,"NULL") = 0
						Тогда
				...
				ИначеЕсли СтрокаМаппинга.Тип = ОписаниеТиповКатегорииАктивов Тогда
					Значение = Справочники.КатегорииАктивов.НайтиПоРеквизиту("КодСправочника",СтрокаТЧ[СтрокаМаппинга.КолонкаФайла]);
				ИначеЕсли СтрокаМаппинга.Тип = ОписаниеТиповДоходы Тогда
					Значение = Справочники.Доходы.НайтиПоРеквизиту("КодСправочника",СтрокаТЧ[СтрокаМаппинга.КолонкаФайла]);
				ИначеЕсли СтрокаМаппинга.Тип = ОписаниеТиповТоварыИУслуги Тогда
					Значение = Справочники.ТоварыИУслуги.НайтиПоРеквизиту("КодСправочника",СтрокаТЧ[СтрокаМаппинга.КолонкаФайла]);
				ИначеЕсли СтрокаМаппинга.Тип = ОписаниеТиповВидыДвижения Тогда
					Значение = Справочники.ВидыДвижения.НайтиПоРеквизиту("КодСправочника",СтрокаТЧ[СтрокаМаппинга.КолонкаФайла]);
				ИначеЕсли СтрокаМаппинга.Тип = ОписаниеТиповВалюты Тогда
					Значение = Справочники.Валюты.НайтиПоРеквизиту("КодСправочника",СтрокаТЧ[СтрокаМаппинга.КолонкаФайла]);
	...
	Для Каждого СтрокаТЧ Из ТаблицаВозврата Цикл
		СтрокаТЧ.Год = НАчалоГода(СтрокаТЧ.Период);
	КонецЦикла;
	
	Адрес = ПоместитьВоВременноеХранилище(ТаблицаВозврата,Адрес);
КонецПроцедуры
