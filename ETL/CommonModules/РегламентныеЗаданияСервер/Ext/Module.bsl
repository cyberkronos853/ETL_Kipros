...
Процедура ОтправкаЭлектроннойПочтыПоЗаявкам() Экспорт
	

	Запрос = Новый Запрос();
	Запрос.Текст = "ВЫБРАТЬ
	               |	ЗаявкаНаДобавлениеСправочника.Ссылка КАК Ссылка,
	               |	ЗаявкаНаДобавлениеСправочника.ТипСправочника КАК ТипСправочника,
	               |	ЗаявкаНаДобавлениеСправочника.Наименование КАК Наименование,
	               |	ЗаявкаНаДобавлениеСправочника.ЭлектронныйАдрес КАК ЭлектронныйАдрес,
	               |	ЗаявкаНаДобавлениеСправочника.Ответственный КАК Ответственный,
	               |	ЗаявкаНаДобавлениеСправочника.НомерЗаявки КАК Номер,
	               |	ЗаявкаНаДобавлениеСправочника.Организация КАК Организация
	               |ИЗ
	               |	Документ.ЗаявкаНаДобавлениеСправочника КАК ЗаявкаНаДобавлениеСправочника
	               |ГДЕ
	               |	ЗаявкаНаДобавлениеСправочника.Статус = ""Создана""";
	Рез = Запрос.Выполнить().Выбрать();
	ЕстьЗаявки = Ложь;
	ТелоПисьма = "";
	Пока рез.Следующий() Цикл
		ТелоПисьма = ТелоПисьма+"Организация: "+рез.Организация+  "№ заявки: "+рез.Номер+  ", Ответственное Лицо: "+рез.Ответственный+", Тип справочника: "+рез.ТипСправочника+", Наименование: " +рез.Наименование+ Символы.ПС;
		ЕстьЗаявки = Истина;
	КонецЦикла;
	...
	Запрос = Новый Запрос();
	Запрос.Текст = "ВЫБРАТЬ
	               |	ПолучателиРассылки.АдресОтправки КАК АдресОтправки
	               |ИЗ
	               |	РегистрСведений.ПолучателиРассылки КАК ПолучателиРассылки
	               |ГДЕ
	               |	ПолучателиРассылки.ТипУведомления = &ТипУведомления"; 
	
	Запрос.УстановитьПараметр("ТипУведомления", Перечисления.ТипыУведомлений.УведомленияОЗаявках);
	
	Рез = Запрос.Выполнить().Выбрать();
	АдресаКоторымОтправляемПисьма = "";
	Пока рез.Следующий() Цикл
		АдресаКоторымОтправляемПисьма = АдресаКоторымОтправляемПисьма + рез.АдресОтправки + ",";
		//АдресаКоторымОтправляемПисьма = "shyngys.nyssanbayev@kipros.kz";
	КонецЦикла;
	...
	ПараметрыПисьма.Вставить("Тема", "Заявка на создание эталонного справочника");	
